...
class manager:
    def __init__(self, path_to_conf):
        ...
        try:
            ...
            self.data = {"hash": None, "chats": [], "emails": [], "name": "USER"}
    ...
    def setup_email(self, email, apikey):
        ...
        self.data["emails"].append({"email": email, "apikey": apikey, "lastuid": None})
        self.save_config()
        return True
    ...
    def receive(self):
        for y, i in enumerate(self.data["emails"]):
            try:
                maybeMessage, lastuid = receive(
                    i["email"], i["apikey"], latest_uid=i["lastuid"]
                )
                    ...
                    for message in maybeMessage:
                        if validate_route(j["routekey"], j["route"], message[0]):
                            if (message[0], 0) not in self.data["chats"][x]["messages"]:
                                self.data["chats"][x]["messages"].append(
                                    (message[0], 0)
                                )
    ...
    def send_message(self, name_of_chat, message, email_to, email_from, password):
        if (
            ...
            try:
                encmessage = encrypt_message(
                    enc_key, chat["routekey"], chat["route"], message
                )
